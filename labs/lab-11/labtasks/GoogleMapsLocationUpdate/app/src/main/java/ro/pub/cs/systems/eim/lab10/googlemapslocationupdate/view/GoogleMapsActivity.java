...
public class GoogleMapsActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == 1) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Log.i(Constants.TAG, "Location permission granted.");
                startLocationUpdates();
            } else {
                Toast.makeText(this, "Aplicația nu poate funcționa fără permisiunea de localizare.", Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private void startLocationUpdates() {
        try {
            ...
            Log.e("[SECURITY]", "startLocationUpdates: Security Exception");
            securityException.printStackTrace();
        } catch (Exception exception) {
            Log.e("[EXCEPTION]", "startLocationUpdates: Exception");
